...
def sign_up(request): 
    ...
    if request.method=="POST":
        if uform.is_valid():
            ...
            return render(request,'registration/sign_up',{'form': uform})
...
def resetPassword(request):
        ...
        try:
            ...
            try:
                ...
                if user is not None:
                    user.set_unusable_password(pwd)
                    user.save()
                    responseDic["errmsg"]="Password Reset Suceessfully"
                    return render (request,"registration/ResetPassword.html",responseDic)
            except Exception as e:
             print(e)
            responseDic["esrrmsg"]="email does not exist"
            return render(request,"registration/ResetPassword.html",responseDic)
